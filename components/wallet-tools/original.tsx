...
// 🛑 อัปเดต: REQUIRED_ENTROPY เป็น 1024 bits (128 bytes)
...
export default function WalletGeneratorPage() {
    ...
    const handleMouseMove = useCallback((event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        ...
        setEntropyPool(prevPool => {
            // 🛑 อัปเดต: เพิ่มขนาดสูงสุดของ Pool เป็น (1024 / 8) + 64 = 192 bytes (เพื่อให้มีข้อมูลมากพอ)
    ...
    const generateWallet = useCallback(() => {
        ...
        setTimeout(() => {
            try {
                // 1. สร้าง Seed หลักจาก window.crypto (256 bits)
                ...
                // Private Key ของ Bitcoin คือ 32 bytes (256 bits) ดังนั้นเราใช้ 32 bytes แรกของ User Entropy Pool
