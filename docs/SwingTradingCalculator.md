# 💡 สูตรคำนวณสำหรับ Swing Trading

เรายังคงใช้หลักการบริหารความเสี่ยงแบบ **Percent of Capital** และ Risk-Reward Ratio แต่ระยะห่างของราคาจะกว้างกว่า Scalping:
...
| **$P_{\text{Entry}}$** | ราคาเข้าซื้อ | $25.00 USD$ |
| **$P_{\text{SL}}$** | ราคา Stop-Loss (ควรตั้งใต้แนวรับสำคัญ) | $24.00 USD$ |
| **$R/R$** | อัตราส่วน Risk-Reward ที่ต้องการ | $2.5:1$ |

**สูตรที่ใช้ในการคำนวณ:** (สูตรเหมือนกับ Day Trading แต่แนวคิดในการตั้ง $P_{\text{SL}}$ ต่างกัน)
...
### 1\. การสร้าง Component `SwingTradingCalculator.tsx`

สร้างไฟล์ใหม่ที่ `src/components/SwingTradingCalculator.tsx` แล้วคัดลอกโค้ดด้านล่างนี้:

```tsx
// src/components/SwingTradingCalculator.tsx
"use client";

import React, { useState, useMemo } from 'react';
import GlassmorphismCard from '@/components/GlassmorphismCard';

export default function SwingTradingCalculator() {
    ...
    const [entryPrice, setEntryPrice] = useState<number | string>(25.00); // P_Entry
    const [stopLossPrice, setStopLossPrice] = useState<number | string>(24.00); // P_SL
    const [riskRewardRatio, setRiskRewardRatio] = useState<number | string>(2.5); // R/R
    ...
    return (
        <GlassmorphismCard className="p-6 md:p-10 w-full max-w-4xl">
            <h3 className="text-3xl font-bold mb-6 text-center text-white">เครื่องมือคำนวณ Swing Trading Position Sizer</h3>
            <p className="text-center text-white/80 mb-8">ใช้สำหรับคำนวณขนาด Position โดยอ้างอิงจากราคา Stop-Loss ที่กำหนด</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                ...
                <div>
                    ...
                    <input
                        ...
                        className={inputClass}
                        placeholder="เช่น 24.00 USD (ใต้แนวรับ)"
                    />
                </div>
                {/* Input R/R */}
                <div className="md:col-span-2">
                    ...
                    <input
                        ...
                        className={inputClass}
                        placeholder="เช่น 2.5 (หมายถึง 2.5:1)"
                    />
                </div>
            </div>
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 border-t border-b border-white/20 py-6 mb-8 text-center">
                        ...
                        <div className="p-3 bg-fuchsia-800/10 rounded-lg">
                            ...
                            {/* บรรทัดที่ 165 ที่เกิด Error 18048: ใช้ results.Q_Size! */}
...
### 2\. การสร้าง Component Page: `src/app/asset/swing-trading/page.tsx`

สร้างโฟลเดอร์ `swing-trading` ภายใน `src/app/asset/` และสร้างไฟล์ `page.tsx` ภายในนั้น:

```tsx
// src/app/asset/swing-trading/page.tsx
import GooeyBackground from '@/components/GooeyBackground';
import SwingTradingCalculator from '@/components/SwingTradingCalculator'; // นำเข้าเครื่องมือคำนวณ Swing Trading
            ...
            <main className="container mx-auto max-w-7xl flex flex-col gap-10">
                ...
                <section className="text-center mb-8">
                    <h1 className="text-5xl font-extrabold mb-4 text-fuchsia-400 drop-shadow-lg">
                        📊 Swing Trading Position Sizer
                    </h1>
                    <p className="text-xl font-light opacity-90 max-w-4xl mx-auto">
                        เครื่องมือคำนวณขนาด Position สำหรับการเทรดระยะกลาง (Swing) 
                        โดยใช้แนวรับ/แนวต้านในการกำหนดจุด Stop-Loss และ Take-Profit
                    </p>
                </section>
...
ตอนนี้คุณมีเครื่องมือคำนวณครบทั้งสี่กลยุทธ์หลักแล้วครับ: **DCA, Scalping, Trend Following,** และ **Swing Trading** ทุกไฟล์ถูกจัดวางอย่างเป็นระเบียบภายใต้ `src/app/asset/`\! หากมีคำถามเพิ่มเติมเกี่ยวกับโค้ดหรือการปรับปรุงส่วนอื่น ๆ แจ้งได้เลยนะครับ\!
